...
import static com.genuitec.eclipse.egit.tools.internal.fbranches.BranchingUtils.*;
...
public class BranchActionsProvider extends CompoundContributionItem implements IWorkbenchContribution {
	...
	private IContributionItem[] createPerBranchContributionItems(List<Repository> repositories, 
		...
		for (String brName: branches) {
			...
			parameter.icon = EGitToolsPlugin.getDefault().getImageRegistry().getDescriptor(
					((mode & MODE_STABLE) != 0) ? EGitToolsPlugin.IMG_BRANCH : EGitToolsPlugin.IMG_FEATURE_BRANCH);
	...
	private class OthersBranchesActionProvider extends ContributionItem {
		...
		public void fill(Menu menu, int index) {
			...
			item.setText(userId);
			item.setImage(EGitToolsPlugin.getDefault().getImageRegistry().get(EGitToolsPlugin.IMG_FEATURE_BRANCH_OWNER));
