...
public class ObsoleteProjectsDialog {
	...
	public ObsoleteProjectsDialog(Shell shell) {
		this.shell = shell;
	}
	
	public List<IProject> select(Set<IProject> obsoleteProjects) {
		...
		dialog.setMessage("Following project has been detected as obsolete, please select those to be removed from your workspace. Note, that their contents will not be removed from the file system.");
		dialog.setTitle("Delete obsolete projects");
		dialog.setInput(obsoleteProjects);
	...
	private class ProjectSelectionContentProvider implements ITreeContentProvider {
		...
		private Set<IProject> obsoleteProjects;
		...
		public void inputChanged(Viewer viewer, Object oldInput, Object newInput) {
			obsoleteProjects = (Set<IProject>) newInput;
		}

		public Object[] getElements(Object inputElement) {
			obsoleteProjects = (Set<IProject>) inputElement;
			...
			for (IProject project: ResourcesPlugin.getWorkspace().getRoot().getProjects()) {
				if (project.isAccessible() && obsoleteProjects.contains(project)) {
			...
			for (IWorkingSet ws: wsm.getWorkingSets()) {
				try {
					...
					for (IAdaptable a: ws.getElements()) {
						...
						if (project != null && project.isAccessible() && obsoleteProjects.contains(project)) {
